...
| raymob | committed snapshot, **patched** | Vendored under `thirdparty/raymob/` (`helper.c`, `display.c`, `soft_keyboard.c`, `callback.c`, `vibrator.c`, `sensor.c`, `raymob.h`) plus the Java side under `raymob/`. The committed files ARE the pin. MIT; the licence header stays at the top of every file it came with. MIT does not require marking changes and every one of them is marked anyway, `[rmp patch]` at the site, because the next person to bump this has to know which lines are not upstream's. |
| raymob — patch (heap overflow) | one allocation | `helper.c` `LoadCacheFile()` allocated `len1 + len2 + 1` for `dir + '/' + name + '\0'`, which needs one more, and then wrote the terminator at `filePath[len]` — one past the end of the block. Under scudo, Android's allocator since API 30, that is an immediate `heap-buffer-overflow WRITE of size 1`; under a quieter one it corrupts the next chunk header and crashes somewhere unrelated later. Now `strlen(dir) + 1 + strlen(name) + 1` and a single `snprintf`, with the malloc checked. `raymob.h` declares the function, so a game can call it. **Re-apply when bumping raymob.** |
| raymob — patch (orientation) | one variable | `display.c` `GetScreenOrientation()` read `if (result >= 0 && result < 4)` where `result` is still 0, so the guard was `0 >= 0 && 0 < 4` — vacuously true — and the bound check never looked at the value it was written to guard. It tests `screenOrientation` now. `DisplayManager.getOrientation()` returns -1 when there is no Display, which is exactly what must not reach the game. **Re-apply when bumping raymob.** |
| raymob — patch (detach) | GetEnv first | `helper.c` `AttachCurrentThread`/`DetachCurrentThread` attached and detached unconditionally. raylib's game loop runs on a thread the VM attached for the life of the process: attaching it again is a no-op, but detaching it is not — it detaches for real, invalidating every local reference the frame above holds. They keep a per-thread depth now and detach only a thread this code attached, only when the outermost pair unwinds. `GetCacheDir()` went through the raw `vm` calls and bypassed all of it; it uses the helpers now. **Re-apply when bumping raymob.** |
| raymob — patch (JNI discipline) | three helpers | `helper.c` gains `RaymobExceptionCheck`, `RaymobGetMethod` and `RaymobGetObjectField` (declared in `raymob.h`), and `display.c`, `soft_keyboard.c`, `vibrator.c`, `callback.c` and `admob.c` go through them. Upstream passed whatever `GetMethodID`/`GetFieldID` returned straight into `CallVoidMethod`/`GetObjectField`/`SetBooleanField`, and checked no `Call*Method` for a pending exception. Both are aborts, not failures: a NULL jfieldID is `JNI DETECTED ERROR IN APPLICATION`, and a pending exception kills the VM at the next JNI call made from any file at all. It is reachable in a release AAB, where R8 renames anything `proguard-rules.pro` does not keep. Gated by `RaymobJniDisciplineTest` in `tests/configure_test.py`. **Re-apply when bumping raymob.** |
| raymob — patch (onKeyUp) | one word | `raymob/app/src/main/java/com/raylib/raymob/NativeLoader.java`'s `onKeyUp` override returned `super.onKeyDown(...)`. Releasing the hardware BACK button sent `NativeActivity` a second down and never an up, so anything pairing them (the back-to-exit gesture, key repeat state) was left holding a key that had been released. **Re-apply when bumping raymob.** |
